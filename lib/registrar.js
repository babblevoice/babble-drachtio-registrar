...
class Registrar {
  ...
  */
   ...
   * <br> - throws an error if the registrar instance has been used more than once
   * <br> - invokes next if the request method is not REGISTER
   * <br> - parses the req.registration.aor property and calls the _isauthed method to get the registration
   * <br> - if the registration is returned, calls reg.onexpire if expired
   * <br>   else calls reg.regping and the _sendok method passing the request and response
   * <br> - else parses the request To header URI, logs it via the options.consolelog method and
   * <br>   passes to the digest authentication function:
   * <br>   - an options object containing the host parsed and a passwordLookup callback
   * <br>   - the request, the response and an onauth callback which:
   * <br>     - sets the req.registrar.contact, .useragent, .allow and .expires properties
   * <br>     - calls res.send and returns if the options.regping method is not present and
   * <br>       the req.registrar.expires property is non-zero and less than the options.minexpires property
   * <br>     - adds a new domain instance by realm if not present
   * <br>     - gets the domain, calls domain.reg and emits the register event with information on the registration
   * <br>     - deletes the domain if no users are present
   * <br>     - calls the _sendok method passing the request and response
   * @method
  ...
  */
  ...
  /**
    ...
    * <br> - sets headers using the request Contact header and options regping property
    * <br>   if the options regping property is present
    * <br> - else sets headers using the request registrar contact and expires properties
    * @private
    * @method
    * @param { Request } req - the incoming request
    * @param { Response } res - the outgoing response
