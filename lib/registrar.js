/*
  ...
  - on        Sets an event listener on the options.em property for an event and a callback passed
  - reg       ...
  - isauthed  Returns a registration by realm and username if present
              if source address, source port and call ID match,
              else returns false
  - realms    Returns an array containing the keys of the domains property
  - users     Returns an array containing info from reg.info for all user registrations at a realm
              or an empty array if no realm
  - user      Parses the host and username from the realm if the username is not passed,
              then returns an array containing info from reg.info for all registrations for the user if present,
...
class Registrar {
  ...
  /*
    Get info for all user registrations at a realm
  */
  users( realm ) {
  ...
  /*
    Get all regs for a user at a realm by preferred call format,
    either user( "bling.babblevoice.com", "1000" )
    or user( "sip:1000@bling.babblevoice.com" )

    Method async for future reg storage in Redis or similar
  */
