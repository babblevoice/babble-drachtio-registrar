...
class Registrar {
  ...
  */
  constructor( options ) {
    ...
    */
    ...
    this.options.srf.use( "register", ( req, res, next ) => this._reg( req, res, next ) )
  ...
  */
  ...
  getauth( req ) {
    ...
    const authreq = sipauth.parseauthheaders( req )
    if( !authreq.username ) return
    if( !store.has( authreq.realm ) ) return


    const dom = store.getdomain( authreq.realm )

    return dom.getauth( authreq )
  }
