...
function deletedomain( reg ) {
  if( bydomain.has( reg._authorization.realm ) ) {
    let d = bydomain.get( reg._authorization.realm )
    d.delete( reg )
    if( 0 === d._users.size ) {
...
module.exports.get = ( req ) => {
  let fqcallid = req.get( "call-id" ) + "@" + req.source_address + ":" + req.source_port
...
module.exports.clear = ( reg ) => {
  bycallidandsource.clear()
  bydomain.clear()
}
