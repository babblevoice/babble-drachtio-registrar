...
class reg {
  ...
  constructor( req, user, singleton ) {
    ...
    if ( undefined !== singleton.options.optionsping ) {
      this.optionsintervaltimer = setInterval( this.pingoptions, singleton.options.optionsping * 1000, this, singleton )
    }

    this.regexpiretimer = setTimeout( this.onexpire, this.expires * 1000, this, singleton.options )
  }
  ...
  pingoptions( self, singleton ) {
    ...
    self.contact.forEach( c => {
      singleton.options.srf.request( c.uri, opts, cb )
    } )
  }
}
