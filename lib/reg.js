...
class reg {
  ...
  constructor( req, user ) {
    ...
    this.divisor = user.options.divisor
    if ( undefined !== user.options.regping ) {
      this.expires = user.options.expires
    }
    this.registeredat = Math.floor( +new Date() / this.divisor )
    this.ping = Math.floor( +new Date() / this.divisor )

    if ( undefined !== user.options.optionsping ) {
      this.optionsintervaltimer = setInterval( this.pingoptions, user.options.optionsping * this.divisor, this )
    }

    this.regexpiretimer = setTimeout( this.onexpire, this.expires * this.divisor, this )
  }

  /** */
  get expired() {
    return ( this.registeredat + this.expires ) < Math.floor( +new Date() / this.divisor )
  }

  /* Expired - but with some buffer */
  /** */
  get expiring() {
    return ( this.registeredat + ( this.expires / 2 ) ) < Math.floor( +new Date() / this.divisor )
  }
  ...
  info() {
    ...
    return {
      ...
      "expiresin": this.registeredat + this.expires - Math.floor( +new Date() / this.divisor ),
      "stale": this.ping < ( Math.floor( +new Date() / this.divisor ) - this.options.staletime )
    }
  }
  ...
  pingoptions( self ) {
    ...
    const cb = ( err, req ) => {
      ...
      req.on( "response", ( res ) => {
        if ( 200 == res.status ) {
          self.ping = Math.floor( +new Date() / this.divisor )
        }
      } )
    }

    self.contact.forEach( c => {
      this.options.srf.request( c.uri, options, cb )
    } )
  }
}
