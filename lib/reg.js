...
class reg {
  ...
  constructor( req, user, registrar ) {
    ...
    if ( undefined !== registrar.options.regping ) {
      this.expires = registrar.options.expires
    }
    this.authorization = user.authorization
    this.user = user
    this.registeredat = Math.floor( +new Date() / 1000 )
    this.ping = Math.floor( +new Date() / 1000 )
    if ( undefined !== registrar.options.optionsping ) {
      this.optionsintervaltimer = setInterval( this.pingoptions, registrar.options.optionsping * 1000, this, registrar )
    }

    this.regexpiretimer = setTimeout( this.onexpire, this.expires * 1000, this, registrar.options )
  }
  ...
  getinfo( options ) {
    ...
    return {
      ...
      "stale": this.ping < ( Math.floor( +new Date() / 1000 ) - options.staletime )
    }
  }
  ...
  update( options ) {
    ...
    this.regexpiretimer = setTimeout( this.onexpire, this.expires * 1000, this, options )
    this.registeredat = Math.floor( +new Date() / 1000 )
    this.initial = false
  }
  ...
  onexpire( self, options ) {
    const ci = self.callid
    self.user.remove( ci, options )
  }

  destroy( options ) {
    options.em.emit( "unregister", this.getinfo( options ) )
    clearInterval( this.optionsintervaltimer )
    clearTimeout( this.regexpiretimer )
  }

  pingoptions( self, registrar ) {
    ...
    const cb = ( err, req ) => {
      if ( err ) {
        registrar.consolelog( `Error sending OPTIONS: ${ err }` )
        return
      }
      req.on( "response", ( res ) => {
        if ( 200 == res.status ) {
    ...
    self.contact.forEach( c => {
      registrar.options.srf.request( c.uri, opts, cb )
    } )
  }
}
